...
class TestAllOperationsOnMV extends QueryTest with BeforeAndAfterEach {
  ...
  test("test materialized view with streaming dmproperty") {
    ...
    intercept[MalformedCarbonCommandException] {
      sql("create materialized view dm mvproperties('STREAMING'='true') as select name from maintable")
  ...
  test("validate dmproperties") {
    ...
    sql("drop materialized view if exists dm ")
    intercept[MalformedCarbonCommandException] {
      sql("create materialized view dm mvproperties('dictionary_include'='name', 'sort_columns'='name') as select name from maintable")
