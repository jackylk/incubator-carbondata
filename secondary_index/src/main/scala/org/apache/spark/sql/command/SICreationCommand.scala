...
import org.apache.spark.sql.hive.{CarbonInternalHiveMetadataUtil, CarbonInternalMetastore}
...
private[sql] case class CreateIndexTable(indexModel: SecondaryIndex,
  ...
  def run(sparkSession: SparkSession): Seq[Row] = {
    ...
    try {
      ...
      var encoders = new ArrayList[Encoding]()
      schemaOrdinal += 1
      ...
      val parentTableIdentifier = new RelationIdentifier(parentCarbonTable.getDatabaseName,
        parentCarbonTable.getTableName,
        parentCarbonTable.getCarbonTableIdentifier.getTableId)

      if (sparkSession.sessionState.catalog.listTables(databaseName)
        .exists(_.table.equalsIgnoreCase(indexTableName))) {
        LOGGER.audit(
          s"Index creation with Database name [$databaseName] and index name [$indexTableName] " +
          s"failed. " +
