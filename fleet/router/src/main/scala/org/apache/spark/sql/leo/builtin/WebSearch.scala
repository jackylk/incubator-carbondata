...
import org.apache.spark.sql.pythonudf.PythonExecUtil
...
case class WebSearchExec(
  ...
  override protected def doExecute(): RDD[InternalRow] = {
    ...
    val rows = lines.map { line =>
      val splited = line.split('|').map(UTF8String.fromString)
      projection(new GenericInternalRow(splited.asInstanceOf[Array[Any]])).copy()
    }
