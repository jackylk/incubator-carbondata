...
class AlterTableTestCase extends QueryTest with BeforeAndAfterAll {
  ...
  test("DeleteCol_001_01", Include) {
    intercept[Exception] {
      sql(s"""create table test1 (name string, id int) STORED AS carbondata TBLPROPERTIES('DICTIONARY_INCLUDE'='id') """).collect
      sql(s"""insert into test1 select 'xx',1""").collect
  ...
  test("DeleteCol_001_04", Include) {
    intercept[Exception] {
      sql(s"""create table test1 (name string, country string, upd_time timestamp, id int) STORED AS carbondata  TBLPROPERTIES('DICTIONARY_INCLUDE'='id')""").collect
      sql(s"""insert into test1 select 'xx','yy',current_timestamp,1""").collect
  ...
  test("AlterTable-001-AltersameTablename-001-TC001", Include) {
   ...
   sql(s"""CREATE TABLE uniqdata (CUST_ID int,CUST_NAME String,ACTIVE_EMUI_VERSION string, DOB timestamp, DOJ timestamp, BIGINT_COLUMN1 bigint,BIGINT_COLUMN2 bigint,DECIMAL_COLUMN1 decimal(30,10), DECIMAL_COLUMN2 decimal(36,10),Double_COLUMN1 double, Double_COLUMN2 double,INTEGER_COLUMN1 int) STORED AS carbondata TBLPROPERTIES('DICTIONARY_INCLUDE'='BIGINT_COLUMN1,BIGINT_COLUMN2,DECIMAL_COLUMN1,DECIMAL_COLUMN2,INTEGER_COLUMN1,CUST_ID')""").collect
