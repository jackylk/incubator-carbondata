...
class StandardPartitionGlobalSortTestCase extends QueryTest with BeforeAndAfterAll {
  ...
  test("partition with int issue and dictionary exclude") {
    try {
      ...
      sql(
        "create table partdatecarb2(name string, dob string) partitioned by(age Int) STORED AS carbondata TBLPROPERTIES('DICTIONARY_EXCLUDE'='age')")
  ...
  test("partition with int issue and dictionary include") {
    ...
    intercept[Exception] {
      sql(
        "create table partdatecarb3(name string, dob string) partitioned by(age Int) STORED AS carbondata TBLPROPERTIES('DICTIONARY_INCLUDE'='age')")
    }
  }
  ...
  test("partition colunm test without partition column in fileheader of load command") {
    ...
    sql("CREATE TABLE partitiontablewithoutpartcolumninfileheader (CUST_ID int,ACTIVE_EMUI_VERSION string, DOB timestamp, DOJ timestamp, BIGINT_COLUMN1 bigint,BIGINT_COLUMN2 bigint,DECIMAL_COLUMN1 decimal(30,10), DECIMAL_COLUMN2 decimal(36,10),Double_COLUMN1 double, Double_COLUMN2 double,INTEGER_COLUMN1 int) partitioned by(CUST_NAME String) STORED AS carbondata TBLPROPERTIES('DICTIONARY_INCLUDE'='CUST_ID,ACTIVE_EMUI_VERSION,DOB,DOJ,BIGINT_COLUMN1,BIGINT_COLUMN2,DECIMAL_COLUMN1,DECIMAL_COLUMN2,Double_COLUMN1,Double_COLUMN2,INTEGER_COLUMN1') ")
  ...
  ignore("data loading with default partition in static partition table with batchsort") {
    ...
    sql(
      """
        ...
        |  utilization int, doj Timestamp, empname String)
        | PARTITIONED BY (projectjoindate Timestamp, salary decimal)
        | STORED AS carbondata TBLPROPERTIES('SORT_SCOPE'='BATCH_SORT')
      """.stripMargin)
