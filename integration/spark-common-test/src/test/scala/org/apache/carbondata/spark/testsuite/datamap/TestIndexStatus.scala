...
import org.apache.carbondata.core.datamap.index.{IndexDistributable, IndexMeta}
...
class TestIndexStatus extends QueryTest with BeforeAndAfterAll {
  ...
  test("datamap status disable for new datamap WITH DEFERRED REFRESH") {
    sql("DROP TABLE IF EXISTS datamapstatustest")
    sql(
      """
        | CREATE TABLE datamapstatustest(id int, name string, city string, age int)
  ...
  test("datamap status disable after new load  WITH DEFERRED REFRESH") {
