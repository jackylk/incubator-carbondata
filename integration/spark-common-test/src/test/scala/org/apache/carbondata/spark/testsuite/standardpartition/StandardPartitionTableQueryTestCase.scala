...
test("Creation of partition table should fail if the colname in table schema and partition column is same even if both are case sensitive"){
  ...
  assert(exception.getMessage.contains(
    "Found duplicate column(s) in the table definition of `default`.`uniqdata_char2`: `name`"))
}
  ...
  ignore("drop partition on preAggregate table should fail"){
    sql("drop table if exists partitionTable")
    sql("create table partitionTable (id int,city string,age int) partitioned by(name string) STORED AS carbondata".stripMargin)
    sql(
  ...
  test("validate data in partition table after dropping and adding a column") {
    ...
    sql("create table par(name string) partitioned by (age double) STORED AS " +
