...
class TestBigDecimal extends QueryTest with BeforeAndAfterAll {
  ...
  override def beforeAll {
    ...
    sql("create table if not exists carbonBigDecimal_3 (ID Int, date Timestamp, country String,name String, phonetype String, serialname String, salary decimal(30, 2)) STORED AS carbondata TBLPROPERTIES('dictionary_include'='salary')")
  ...
  test("test big decimal for dictionary look up") {
    ...
    sql("create table if not exists decimalDictLookUp (ID Int, date Timestamp, country String, name String, phonetype String, serialname String, salary decimal(27, 10)) STORED AS carbondata TBLPROPERTIES('dictionary_include'='salary')")
