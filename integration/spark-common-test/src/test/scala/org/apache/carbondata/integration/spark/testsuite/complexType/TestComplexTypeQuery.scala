...
class TestComplexTypeQuery extends QueryTest with BeforeAndAfterAll {
  ...
  override def beforeAll: Unit = {
    ...
    sql(
      ...
      "array<int>,test5 array<string>,test6 array<timestamp>,test7 array<string>) STORED AS carbondata")
  ...
  test("test for create table with complex type") {
    try {
      ...
      sql(
        ("CREATE TABLE CARBON_TABLE(stringField string,complexData array<string>)STORED AS " +
         "carbondata ")
          .stripMargin)
      assert(true)
    }
  ...
  test(
    ...
    sql(
      ...
      "activeDeactivedate:array<string>>, gamePointId double,contractNumber double)  STORED AS " +
      "carbondata");
    sql("LOAD DATA local inpath '" + resourcesPath +
