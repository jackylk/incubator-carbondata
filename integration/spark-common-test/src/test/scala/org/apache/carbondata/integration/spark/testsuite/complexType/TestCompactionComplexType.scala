...
class TestCompactionComplexType extends QueryTest with BeforeAndAfterAll {
  ...
  test("test Decimal with Struct") {
    sql("Drop table if exists adaptive")
    sql(
      "create table adaptive(roll int, student struct<id:decimal(3,2),name:string>) STORED AS " +
      "carbondata")
  ...
  test("test Timestamp with Struct") {
    ...
    sql(
      "create table adaptive(roll int, student struct<id:timestamp,name:string>) STORED AS " +
      "carbondata")
