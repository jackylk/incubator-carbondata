...
class SubQueryJoinTestSuite extends Spark2QueryTest with BeforeAndAfterAll {
  ...
  test("test to check if 2nd level subquery gives correct result") {
    ...
    sql("create table t1 (s1 string) STORED AS carbondata tblproperties('dictionary_include'='s1')")
  ...
  test("test join with dictionary include with udf") {
    ...
    sql(
      ...
      "quantity double, unit smallint, b_country smallint, imex int, y_year smallint) STORED AS " +
    ...
    sql(
      "create table t2(id bigint, hs string, hs_cn string, hs_en string) STORED AS carbondata " +
