 ...
 */
  def apply(plan: LogicalPlan): Seq[SparkPlan] = {
    plan match {
      ...
      case InternalProject(tid, child: LogicalPlan) =>
        CarbonInternalProject(tid, planLater(child)) :: Nil
      case _ => Nil
    }
  }
  ...
  private def getDataSourceScan(relation: LogicalRelation,
    ...
    if (supportBatchedDataSource(relation.relation.sqlContext, updateRequestedColumns,
      ...
      val scan = CarbonReflectionUtils.getRowDataSourceScanExecObj(relation, output,
        pushedFilters, handledFilters,
        rdd, partition, metadata)
      scan.relation.asInstanceOf[CarbonDatasourceHadoopRelation].needPriv = relation.needPriv
      scan
    }
  }
