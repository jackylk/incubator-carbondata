 ...
 */
  ...
  protected def pruneFilterProjectRaw(
    ...
    if (projects.map(_.toAttribute) == projects &&
      ...
      val updateRequestedColumns = updateRequestedColumnsFunc(requestedColumns, table)
      ...
      var updateRequestedColumns =
        ...
        updateRequestedColumnsFunc(requestedColumns, table)
      }
      val supportBatch =
      ...
      if (!vectorPushRowFilters && !supportBatch && !implicitExisted && !hasDictionaryFilterCols
          && !hasMoreDictionaryCols) {
        // revert for row scan
        updateRequestedColumns = updateRequestedColumnsFunc(requestedColumns, table)
      }
      ...
      if (!vectorPushRowFilters && scan.isInstanceOf[CarbonDataSourceScan]
        ...
        execution.ProjectExec(
          updateRequestedColumnsFunc(updatedProjects, table).asInstanceOf[Seq[NamedExpression]],
  ...
  def updateRequestedColumnsFunc(requestedColumns: Seq[Expression],
      relation: CarbonDatasourceHadoopRelation): Seq[Expression] = {
    requestedColumns
  }
