...
case class CarbonCollectLoadsCommand(
  ...
  override def processMetadata(sparkSession: SparkSession): Seq[Row] = {
    ...
    val loadDetailsHistoryDir = CarbonTablePath.getLoadDetailsHistoryDir(tablePath) + "/" + timestamp
    ...
    try {
      LOGGER.info("start to collect segments, timestamp: " + timestamp)
      ...
      if (detailFiles.isEmpty) {
        ...
        readLoadDetailsDir(executorService, detailFiles, detailList)
  ...
  private def readLoadDetailsDir(
  ...
  private def updateTableStatus(
    ...
    val tablestatusLock = new SegmentStatusManager(carbonTable.getAbsoluteTableIdentifier).getTableStatusLock()
    try {
      if (tablestatusLock.lockWithRetries) {
        LOGGER.info(
          "Acquired lock for table" + carbonTable.getDatabaseName() + "." + carbonTable.getTableName()
        ...
        LOGGER.error(
          "Not able to acquire the lock for Table status updation during collecting segment for table " +
            carbonTable.getDatabaseName() + "." + carbonTable.getTableName());
      }
    } finally {
      if (tablestatusLock.unlock) {
        LOGGER.info("Table unlocked successfully after table status updation during collecting segments" +
          carbonTable.getDatabaseName + "." + carbonTable.getTableName)
      } else {
        LOGGER.error("Unable to unlock Table lock for table" + carbonTable.getDatabaseName + "." +
          carbonTable.getTableName + " after table status updation during collect segments")
      }
    }
  }
  ...
  private def listLoadDetails(
                               loadDetailsDir: String,
                               hadoopConf: Configuration): Array[(CarbonFile, CarbonFile)] = {
    val dir = FileFactory.getCarbonFile(loadDetailsDir, hadoopConf)
    if (dir.exists()) {
      val allFiles = dir.listFiles()
      val successFiles = allFiles.filter { file =>
        file.getName.endsWith(CarbonTablePath.LOAD_DETAIL_SUBFIX)
      }.map { file =>
  ...
  private def acquiredTableLock(table: CarbonTable): ICarbonLock = {
    ...
    val lock = CarbonLockFactory.getCarbonLockObj(tableIdentifier, "table_collect_segments.lock")
