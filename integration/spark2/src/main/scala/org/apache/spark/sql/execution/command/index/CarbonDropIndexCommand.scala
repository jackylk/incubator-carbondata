      ...
      try {
        ...
        case ex: NoSuchTableException =>
          throwMetadataException(dbName, tableName,
            s"Dropping datamap $indexName failed: ${ ex.getMessage }")
      }
    }
    ...
    try {
      ...
      if (mainTable != null &&
        ...
        val isDMSchemaExists = DataMapStoreManager.getInstance().getAllDataMap(mainTable).asScala.
          exists(_.getDataMapSchema.getDataMapName.equalsIgnoreCase(indexName))
        if (isDMSchemaExists) {
          dropSchemaFromSystemFolder(sparkSession)
          return Seq.empty
        }
        ...
        val dmSchema = DataMapStoreManager.getInstance().getAllDataMapSchemas.asScala
          .filter(dataMapSchema => dataMapSchema.getDataMapName.equalsIgnoreCase(indexName))
      ...
      case e: NoSuchDataMapException =>
        throw e
      case ex: Exception =>
        LOGGER.error(s"Dropping datamap $indexName failed", ex)
        throwMetadataException(dbName, tableName,
          s"Dropping datamap $indexName failed: ${ ex.getMessage }")
    }
    finally {
