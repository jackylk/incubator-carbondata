...
public final class SegmentManager {
    ...
    public static CarbonFile createSegmentFile(CarbonTable table, Map<String, String> options) throws IOException {
        if (options == null) {
        ...
        final Segment segment = new Segment(
            ...
            SegmentFileStore.genSegmentFileName(
                segmentIdentifier,
                loadMetadataDetailsFileName) + CarbonTablePath.SEGMENT_EXT,
            segmentPath,
            options);
    ...
    public static void deleteSegmentFileQuietly(final CarbonFile segmentFile) {
        try {
            ...
            LOGGER.error("Fail to delete segment file [" + segmentFile + "].", exception);
        }
    }
    ...
    private static CarbonFile createLoadMetadataDetails(
        ...
        final HashMap<String, Long> dataSizeAndIndexSize = CarbonUtil.getDataSizeAndIndexSize(segmentPath);
        // Set empty load name, it will be replaced at compact.
        loadMetadataDetails.setLoadName("");
        loadMetadataDetails.setSegmentStatus(SegmentStatus.SUCCESS);
        loadMetadataDetails.setDataSize(dataSizeAndIndexSize.get(CarbonCommonConstants.CARBON_TOTAL_DATA_SIZE).toString());
        loadMetadataDetails.setIndexSize(dataSizeAndIndexSize.get(CarbonCommonConstants.CARBON_TOTAL_INDEX_SIZE).toString());
