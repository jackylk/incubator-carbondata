 ...
 *   For the above query, we create index expression as AndDataMapExpression(index1, index2).
...
public class IndexChooser {
  ...
  private List<TableIndex> cgDataMaps;
  private List<TableIndex> fgDataMaps;
  ...
  public IndexChooser(CarbonTable carbonTable) throws IOException {
    ...
    for (TableIndex visibleDataMap : visibleDataMaps) {
      ...
      if (status != null && status.isEnabled()) {
        IndexLevel level = visibleDataMap.getIndexFactory().getIndexLevel();
  ...
  DataMapExprWrapper chooseDataMap(IndexLevel level, FilterResolverIntf resolverIntf) {
    if (resolverIntf != null) {
      Expression expression = resolverIntf.getFilterExpression();
      List<TableIndex> datamaps = level == IndexLevel.CG ? cgDataMaps : fgDataMaps;
      if (datamaps.size() > 0) {
  ...
  private static class DataMapTuple implements Comparable<DataMapTuple> {
    ...
    public DataMapTuple(int order, TableIndex dataMap) {
      this.order = order;
