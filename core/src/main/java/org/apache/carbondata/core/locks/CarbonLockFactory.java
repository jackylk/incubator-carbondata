...
public class CarbonLockFactory {
  ...
  private static void getLockTypeConfigured() {
    ...
    String lockClassName = CarbonProperties.getInstance().getProperty(CarbonCommonConstants.LOCK_CLASS);
  ...
  private static ICarbonLock newCustomLock(final String lockFileLocation, final String lockFile) {
    if (lockConstructor == null) {
      throw new IllegalArgumentException("Carbon property [" + CarbonCommonConstants.LOCK_CLASS + "] is not set.");
    }
    try {
