...
public class CacheProvider {
  ...
  public <K, V> Cache<K, V> createCache(CacheType cacheType) {
    ...
    if (!isExists(cacheType)) {
      synchronized (lock) {
        if (!isExists(cacheType)) {
          if (null == carbonLRUCache) {
            createLRULevelCacheInstance();
          }
          createBlockletCache(cacheType);
        }
      }
    }
   ...
   * @param cacheType       type of cache
   */
  private void createBlockletCache(CacheType cacheType) {
    Cache cacheObject = new BlockletDataMapIndexStore(carbonLRUCache);
    cacheTypeToCacheMap.put(cacheType, cacheObject);
  }
   ...
   * @return
   */
  private boolean isExists(CacheType cacheType) {
